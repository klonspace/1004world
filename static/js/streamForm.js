    ...
    function initSubmitHandler(status) {
        document.getElementById('streamFormEdit').addEventListener("submit", async function (e) {
            ...
            if (CKEDITOR.instances.streamDesc.getData() != "" &&
                ...
                //     streamEnd: streamEnd,
                //     streamImage: document.getElementById("streamImage").files[0]
                // }
                const formdat = new FormData();

                
                formdat.append('streamID', document.getElementById("streamID").value)
                formdat.append('streamTitle', document.getElementById("streamTitle").value)
                formdat.append('streamDesc', CKEDITOR.instances.streamDesc.getData())
                formdat.append('streamStart', streamStart)
                formdat.append('streamEnd', streamEnd)
                formdat.append('streamImage', document.getElementById("streamImage").files[0])
                ...
                var success
                if (status == "addstream") success = await postDataForm('/addStream', formdat);
                else success = await postDataForm('/editStream', formdat);
                console.log(success)
